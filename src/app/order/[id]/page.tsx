            ...
            <div className="min-h-screen bg-[#FDFDFD] flex flex-col font-sans text-sans">
                ...
                <main className="flex-grow container mx-auto px-4 py-20 max-w-2xl text-center">
                    <div className="bg-white rounded-3xl p-12 shadow-sm border border-gray-100">
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 mb-4 tracking-tight">Không tìm thấy Đơn hàng</h1>
        ...
        <div className="min-h-screen bg-[#FDFDFD] flex flex-col font-sans">
            ...
            <main className="flex-grow container mx-auto px-4 py-8 max-w-6xl">
                ...
                <div className="text-sm font-medium text-gray-500 mb-6 flex items-center gap-2">
                    ...
                    <span className="text-gray-900">Theo dõi đơn hàng</span>
                </div>


                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">Đơn hàng {params.id ? `#${params.id}` : ''}</h1>
                        <p className="text-gray-500 mt-1">Thời gian đặt: <span className="font-bold text-orange-500">{currentOrder?.displayTime || currentOrder?.time || "--:--"}</span></p>
                    </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                            <h2 className="text-lg font-bold text-gray-900 mb-8 flex items-center gap-2">
                                <span className="w-1.5 h-6 bg-orange-50 rounded-full inline-block"></span>
                                Trạng thái đơn hàng
                            </h2>
                            ...
                            <div className="relative flex justify-between items-start">
                                ...
                                <div
                                    className={`absolute top-6 left-[10%] h-1 rounded-full -z-10 transition-all duration-700 ease-in-out ${['đang chờ', 'đã xác nhận'].includes(currentStatus) ? 'w-0' :
                                        currentStatus === 'đang chuẩn bị' ? 'w-[40%] bg-orange-400' :
                                            ['đang giao', 'đã hoàn thành'].includes(currentStatus) ? 'w-[80%] bg-green-500' : 'w-0'
                                        }`}
                                ></div>


                                <div className="flex flex-col items-center w-1/3 z-10">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center shadow-lg mb-3 border-4 border-white transition-all ${currentStatus === 'đang chờ' ? 'bg-orange-100 text-orange-500 shadow-orange-50' :
                                        'bg-green-500 text-white shadow-green-200'
                                        }`}>
                                        {currentStatus === 'đang chờ' ? <Clock className="w-6 h-6" /> : <Check className="w-6 h-6" />}
                                    </div>
                                    <h3 className={`font-bold text-center transition-colors ${currentStatus === 'đang chờ' ? 'text-orange-500' : 'text-gray-900'
                                        }`}>
                                        {currentStatus === 'đang chờ' ? 'Chờ xác nhận' : 'Đã xác nhận'}
                                    </h3>
                                    <p className="text-xs text-gray-500 text-center mt-1">{currentOrder?.displayTime || "--:--"}</p>
                                </div>


                                <div className="flex flex-col items-center w-1/3 z-10">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 border-4 border-white relative transition-all duration-500 ${['đang chờ', 'đã xác nhận'].includes(currentStatus) ? 'bg-gray-100 text-gray-400' :
                                        currentStatus === 'đang chuẩn bị' ? 'bg-orange-200 text-orange-600 shadow-lg shadow-orange-100' :
                                            'bg-green-500 text-white shadow-lg shadow-green-200'
                                        }`}>
                                        {['đang giao', 'đã hoàn thành'].includes(currentStatus) ? <Check className="w-6 h-6" /> : <ChefHat className="w-6 h-6" />}
                                        {currentStatus === 'đang chuẩn bị' && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-white animate-pulse"></span>}
                                    </div>
                                    <h3 className={`font-bold text-center transition-colors ${currentStatus === 'đang chuẩn bị' ? 'text-orange-600' :
                                        ['đang chờ', 'đã xác nhận'].includes(currentStatus) ? 'text-gray-400' :
                                            'text-gray-900'
                                        }`}>Đang chuẩn bị</h3>
                                    {currentStatus === 'đang chuẩn bị' && (
                                        <>
                                            <p className="text-xs text-orange-500/80 text-center mt-1 font-medium select-none">Đang thực hiện</p>
                                        </>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col items-center w-1/3 z-10">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 border-4 border-white relative transition-all duration-500 ${['đang chờ', 'đã xác nhận', 'đang chuẩn bị'].includes(currentStatus) ? 'bg-gray-100 text-gray-400' :
                                        currentStatus === 'đang giao' ? 'bg-blue-200 text-blue-600 shadow-lg shadow-blue-100' :
                                            'bg-green-500 text-white shadow-lg shadow-green-200'
                                        }`}>
                                        {currentStatus === 'đã hoàn thành' ? <Check className="w-6 h-6" /> : <Bike className="w-6 h-6" />}
                                        {currentStatus === 'đang giao' && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-white animate-pulse"></span>}
                                    </div>
                                    <h3 className={`font-bold text-center transition-colors ${currentStatus === 'đang giao' ? 'text-blue-600' :
                                        ['đang chờ', 'đã xác nhận', 'đang chuẩn bị'].includes(currentStatus) ? 'text-gray-400' :
                                            'text-gray-900'
                                        }`}>
                                        {currentStatus === 'đã hoàn thành' ? 'Đã giao' : 'Đang giao'}
                                    </h3>
                                    {['đang chờ', 'đã xác nhận', 'đang chuẩn bị'].includes(currentStatus) && <p className="text-xs text-gray-400 text-center mt-1">Đang chờ</p>}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                            <h2 className="text-lg font-bold text-gray-900 mb-6">Món đã đặt</h2>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            <div className="p-5 flex justify-between items-center bg-white border-b border-gray-50 z-10 relative">
                                <h3 className="font-bold text-gray-900">Vị trí tài xế</h3>
                                <span className={`text-xs px-2 py-1 rounded-full font-bold uppercase tracking-widest flex items-center gap-1 transition-colors ${currentStatus === 'đang giao' ? 'bg-red-50 text-red-500' : 'bg-gray-100 text-gray-500'}`}>
                                    {currentStatus === 'đang giao' && <span className="w-1.5 h-1.5 rounded-full bg-red-500 inline-block animate-pulse"></span>}
                                    {currentStatus === 'đang giao' ? 'Trực tiếp' : currentStatus === 'đã hoàn thành' ? 'Hoàn thành' : 'Ngoại tuyến'}
                                </span>
                            </div>
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
                            <h3 className="font-bold text-gray-900 mb-5">Thông tin giao hàng</h3>
                            ...
                            <div className="space-y-6">
                                <div className="flex gap-4 items-start">
                                    ...
                                    <div>
                                        <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Địa chỉ giao hàng</div>
                                        <p className="text-sm font-semibold text-gray-900 leading-snug">
                                            {currentOrder?.address || "Nhận tại quầy / Chưa cập nhật"}
                                        </p>
                                        <p className="text-xs text-gray-500 mt-1 italic">Theo phương thức đã chọn khi thanh toán.</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-4 items-start">
                                    ...
                                    <div>
                                        <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Thời gian Đặt</div>
                                ...
                                <div className="flex gap-4 items-start">
                                    ...
                                    <div>
                                        <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Phương thức Thanh toán</div>
                                        <div className="flex items-center gap-2">
                                            <p className="text-sm font-semibold text-gray-900">{currentOrder?.paymentMethod === 'momo' ? 'Ví MoMo' : (currentOrder?.paymentMethod === 'vnpay' ? 'VNPay' : 'Tiền mặt/Ngân hàng')}</p>
